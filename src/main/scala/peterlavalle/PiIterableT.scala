...
trait PiIterableT {
	...
	implicit class PiIterable[T](iterable: Iterable[T]) {
		...
		def dropSame(them: Iterable[T]): (Stream[T], Stream[T]) =
			(iterable.toStream, them.toStream) match {
				case (lHead #:: lTail, rHead #:: rTail) if lHead == rHead =>
					lTail dropSame rTail
				case e =>
					e
			}

		def leadBy(them: Iterable[T]): Boolean = (this dropSame them)._2.isEmpty
